...
class myDataset(Dataset):
    def __init__(self, data_path, transform=None, label_mappings=None):
        ...
        for idx in range(len(self.name_list)):
            ...
            if any(task in info_dict for task in self.label_mappings.keys()):
    ...
    def __getitem__(self, idx):
        ...
        for task in self.label_mappings.keys():
            if task in info_dict: 
                value = info_dict[task]
                labels[task] = torch.zeros(len(self.label_mappings[task]), dtype=torch.float32)
                for finding in value:
                    if finding in self.label_mappings[task]:
                        labels[task][self.label_mappings[task][finding]] = 1.0
