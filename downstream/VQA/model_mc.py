...
class ViltWithBackbone(ViltForQuestionAnswering):
    def __init__(self, config, vision_backbone,class_num):
        ...
        self.backbone = vision_backbone  # 使用自定义的ViT模型
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, pixel_values=None, labels=None):
        ...
        if labels is not None:
            loss = loss_fct(logits[self.question_topic].view(-1, self.classnum), labels.view(1,-1))
        return logits, loss
...
class MultiTaskModel(nn.Module):
    ...
    def forward(self, image,question,labels,size=(224,224)):
        ...
        logits, loss = self.model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=pixel_values,labels=torch.tensor(labels).to(self.model.device))
        return logits, loss
